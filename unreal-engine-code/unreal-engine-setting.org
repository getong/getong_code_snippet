...
#+begin_src sh
cd /Users/Shared/Epic\ Games/UE_5.8/Engine/Build/BatchFiles/Mac
...
## macos
ue4 setroot /Users/Shared/Epic\ Games/UE_5.8
## linux
...
# Expand ue4cli
ue() {
  ue4cli=/Users/gerald/.local/pipx/venvs/ue4cli/bin/ue4
  ## linux
  ...
  if [[ "$1" == "engine" ]]; then
    ...
    local uprojects=( *.uproject(N) )
    if (( ${#uprojects} != 1 )); then
      print -u2 "ue gen: expected exactly one .uproject in $PWD, found ${#uprojects}"
      return 1
    fi

    "$ue4cli" gen -vscode || return
    local project="${uprojects[1]:r}"
    local compile_db=".vscode/compileCommands_${project}.json"
    if [[ ! -f "$compile_db" ]]; then
      print -u2 "ue gen: compile database was not generated: $compile_db"
      return 1
    fi
...
if not isinstance(entries, list):
  raise SystemExit(f"{src} is not a compile_commands.json array")
bad = []
for entry in entries:
  if not isinstance(entry, dict):
    bad.append("<non-object entry>")
    continue
  if not entry.get("file") or not (entry.get("arguments") or entry.get("command")):
    bad.append(entry.get("file", "<unknown>"))
if bad:
  raise SystemExit("invalid compile_commands.json entries: " + ", ".join(bad[:5]))
for entry in entries:
  args = entry.get("arguments")
  if args and "-std=c++20" not in args:
    entry["arguments"] = [args[0], "-std=c++20", *args[1:]]
with open(dst, "w") as f:
  json.dump(entries, f, indent=2)
  f.write("\n")
print(f"wrote {dst}: {len(entries)} entries copied from {src}")' "$compile_db" compile_commands.json || return
...
#+begin_src sh
# /Users/Shared/Epic\ Games/UE_5.8/Engine/Build/BatchFiles/Mac/GenerateProjectFiles.sh -project="/Users/gerald/Documents/Unreal Projects/MyProject/MyProject.uproject" -game -vscode

#/usr/local/bin/ue4 gen -vscode
~/.local/pipx/venvs/ue4cli/bin/ue4 gen -vscode
