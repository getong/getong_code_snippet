...
*** the service selector project must be the same with pod project name

Currently, the `project-name` should be changed within the pod and service yaml config file.
...
#+begin_src shell
kubectl get deployment project-deploy
#+end_src

*** delete a deployment:

#+begin_src shell
kubectl delete deployment project-deploy
#+end_src
...
#+begin_src shell
kubectl delete pod project-deploy-85dffd5d64-4rbdr
#+end_src
...
#+begin_src shell
kubectl scale --replicas 5 deployment/project-deploy
#+end_src
